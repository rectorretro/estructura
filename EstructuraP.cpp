...
void restarCantidad(int a)
{
    ...
    {
        int Clavex = listaProductos[i].Clave;
    ...
    int cantidadx = listaProductos[i].cantidad;
    double Preciox = listaProductos[i].Precio;

        if(listaProductos[i].Clave == a )
        {
                if(cantidadx == 0) {
...
void DatosCompradas(int i)
{
    ...
    string Tipox = listaProductos[i].Tipo;
    string Contenidox = listaProductos[i].Contenido;
    double cantidadx = listaProductos[i].cantidad;
    double Preciox = listaProductos[i].Precio;
...
void insertarC(int Clavex,string NombreProductox,string Distribuidorx,string Tipox,string Contenidox,int cantidadx, double Preciox)
{
    ...
    listaCompradas[lugarVacio].Clave = Clavex;
    ...
    listaCompradas[lugarVacio].cantidad= cantidadx;
    listaCompradas[lugarVacio].Precio = Preciox;
}
...
void mostrarDatosC()
{
    ...
    for(int i=0; i< NUM; i++)
    {
        if (listaCompradas[i].Clave !=0)
        {
            cout << setw(6) << listaCompradas[i].Clave
                 ...
                 << setw(15) << listaCompradas[i].Precio  << "$"<< endl;
        }
    }
}
...
void escribirEnArchivoComp()
{
    ...
    for(int i=0; i<NUM; i++)
    {
        if (listaCompradas[i].Clave != 0)
        {
            salidaArchivo << listaCompradas[i].Clave << ' '
                          ...
                          << listaCompradas[i].cantidad << ' '
                          << listaCompradas[i].Precio << ' ' << endl;
...
void pedirDatos()
{
    ...
    for(int i = 0; i< NUM-1; i++)
    {
        arreglo[i]=listaProductos[i].Clave;
    }
...
void insertar(int Clavex,string NombreProductox,string Distribuidorx,string Tipox,string Contenidox,int cantidadx, double Preciox)
{
    ...
    listaProductos[lugarVacio].Clave = Clavex;
    ...
    listaProductos[lugarVacio].cantidad= cantidadx;
    listaProductos[lugarVacio].Precio = Preciox;
}
...
void mostrarDatos()
{
    ...
    for(int i=0; i< NUM; i++)
    {
        if (listaProductos[i].Clave !=0)
        {
            cout << setw(6) << listaProductos[i].Clave
                 ...
                 << setw(14)  << listaProductos[i].cantidad
                 << setw(15) << listaProductos[i].Precio  << "$"<< endl;
        }
    }
}
...
void escribirEnArchivo()
{
    ...
    for(int i=0; i<NUM; i++)
    {
        if (listaProductos[i].Clave != 0)
        {
            salidaArchivo << listaProductos[i].Clave << ' '
...
void ordenacionBurbujaPorClave()
{
    for (int i=0; i < NUM-1; i++)
    {
        for (int j=0; j < NUM-1; j++)
        {
            if (listaProductos[j].Clave > listaProductos[j+1].Clave)
            {
                //intercambio
                aux = listaProductos[j];
                listaProductos[j] = listaProductos[j+1];
...
void ordenacionBurbujaPorPrecio()
{
    for (int i=0; i<NUM-1; i++)
    {
        for (int j=0; j<NUM-1; j++)
        {
            if(listaProductos[i].Precio > listaProductos[i].Precio)
            {
                //intercambio
                aux=listaProductos[j];
                listaProductos[j]=listaProductos[j+1];
...
int busquedaBinaria(int b)
{
    ...
    {
        central = (bajo+alto)/2;
        if (b == listaProductos[central].Clave)
        {
            return central;
        }
        else if (b < listaProductos[central].Clave)
        {
            alto = central -1;
        }
        else
...
void restarsaldo(int buscar)
{
    ...
    if (listaProductos[resultado].Precio > compras[0].saldo)
    {
    ...
    else
    {
        ...
        compras[0].saldo=compras[0].saldo-listaProductos[resultado].Precio;
        escribirEnArchivoS();
    }
